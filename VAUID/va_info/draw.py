...
from ..utils.api.model.asset import SkinData, AgentData, CollectionData
...
async def draw_va_info_img(
    ...
    DrawUtils.draw_hexagonal_panel(
        ...
        base_image,
        fill_color=(100, 149, 237, 180),
        outline_color=(65, 105, 225, 255),
    )

    # 绘制数据标签
    draw_base = ImageDraw.Draw(base_image)
    center_x, center_y = base_image.size[0] // 2 + 20, base_image.size[1] // 2 - 50
...
async def draw_asset_section(
    ...
    max_items: int = 20,
    items_per_row: int = 5,
    item_spacing: int = 220,
    section_bottom_offset: int = 50,
) -> int:
    """绘制资产分类区域
